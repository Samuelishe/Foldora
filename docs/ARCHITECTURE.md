...
Dev/manual publish layout создаётся script-ом `scripts/publish-dev.ps1` в `artifacts/publish/Foldora`, где `Foldora.App.exe`, `Foldora.Cli.exe` и `Foldora.MenuHost.exe` лежат рядом. WPF `ExplorerCommandHostPathResolver` сначала ищет sibling `Foldora.MenuHost.exe` рядом с текущим executable, затем Debug build output для Rider/dev workflow. Если host не найден, resolver выдаёт controlled failure; он не должен silently fallback-ить на console `Foldora.Cli.exe` или несуществующий путь.
