...
`DesktopIniService` применяет атрибуты через `DesktopIniAttributePolicy`. Default policy остаётся `CompatibilitySystem` для совместимости с текущим ручным MVP-поведением: folder `System`, `desktop.ini` `Hidden + System`. Альтернативные policies существуют для manual-test-driven investigation deletion UX и передаются через `DesktopIniOptions`; CLI/MenuHost/WPF production flows не выбирают policy сами.
