...
Future image-to-ICO conversion will change the user-facing picker workflow, not the saved menu entry contract. Planned phase 1 input images are `.png`, `.jpg`, `.jpeg` and `.bmp`; Foldora should convert them into generated multi-size `.ico` files and then store normal `IconPath` values. SVG support is separate research and must not be mixed into the first PNG/JPG/BMP milestone.
