...
Saved menu entries accept only real `.ico` paths: файл должен существовать, читаться, быть не пустым, быть не больше 10 MB и иметь корректную ICO-структуру header/directory. PNG/JPG/BMP не сохраняются как `IconPath` напрямую и не принимаются при переименовании в `.ico`.

Image-to-ICO conversion changes the user-facing picker/drop workflow, not the saved menu entry contract. App picker/drop input images `.png`, `.jpg`, `.jpeg` and `.bmp` are converted into generated multi-size `.ico` files and then stored as normal `IconPath` values. SVG support is separate research and must not be mixed into the PNG/JPG/BMP workflow.
...
## Pack Import / Export Future

Pack import/export is useful but lower priority than image-to-ICO conversion and drag/drop icon UX.
...
Import should unpack to temp, validate manifest/icons/file limits, copy selected icons into AppData, generate new entry ids and support merge/replace strategy only with explicit confirmation. Export should collect selected/current entries, copy used icons, write manifest and zip into `.foldorapack`.

Packs are user-provided content unless Foldora explicitly ships an official pack. Foldora must not imply ownership over imported third-party icons, and official bundled packs would require license review and notices.
