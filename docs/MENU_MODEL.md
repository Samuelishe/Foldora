...
Near future перед full tree migration: простая one-level grouping через future поле `FolderMenuEntry.GroupName`. Это должно покрыть базовый UX групп без немедленного перехода на полноценную tree-модель:
...
На текущем этапе storage остаётся flat; `GroupName`, grouping UI и nested runtime/storage migration не реализованы.
