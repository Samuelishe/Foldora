...
Пользовательские menu entries - главный MVP-объект Foldora. Пользователь сам выбирает любые `.ico` и любые подписи; Foldora запрещает только технически опасные значения, которые ломают файловую систему, настройки, реестр или Проводник.
