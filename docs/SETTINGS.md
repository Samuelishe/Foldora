...
- `Help`: краткая Help/About user-facing справка по workflow, `.ico`, Explorer menu, MenuHost, data paths, uninstall and license/resource policy.
- `Danger zone`: reset menu с checkbox-подтверждением. Эта вкладка изолирует destructive reset от default settings view.

SettingsWindow остаётся resizable, но width/minimum width заданы как практический desktop минимум для tab row и RU/EN Explorer/path action rows (`Width=940`, `MinWidth=920`). `SizeToContent` intentionally remains `Manual`: Settings has a fixed footer and tab-local scrolling, so dynamic WPF sizing by selected tab would make the modal window jump and feel brittle. Tab headers size to content and can wrap instead of clipping localized category labels. Header text is centered inside the tab header chrome, but selected tab content stretches through the shared tab style so the tab body behaves like a desktop property page. Inner forms/cards start at the left/top inset; constrained cards such as Danger zone keep readable width but align left. Button layout contract общий для WPF: action buttons size to content through shared padding/min-width rather than fixed local widths. Explorer menu actions use a wrapping row; visible enable/disable labels are intentionally short (`Enable`/`Disable`, `Включить`/`Выключить`) because the tab/section already establishes Explorer menu context; tooltips/help text carry the longer legacy-menu explanation. Installation path rows keep path text in a star-sized column and Open/Copy actions in an auto-sized column so long paths do not squeeze buttons.
