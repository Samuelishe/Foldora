...
SettingsWindow остаётся resizable, но minimum width задан как практический минимум для tab row и RU/EN Explorer/path action rows. Tab headers size to content and can wrap instead of clipping localized category labels. Tab body content starts at a left/top inset like a desktop property page; constrained cards such as Danger zone keep readable width but align left. Button layout contract общий для WPF: action buttons size to content through shared padding/min-width rather than fixed local widths. Explorer menu actions use a wrapping row; Installation path rows keep path text in a star-sized column and Open/Copy actions in an auto-sized column so long paths do not squeeze buttons.
