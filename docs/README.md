...
10. `MENU_MODEL.md`, `PACKS.md`, `SETTINGS.md`, `LOCALIZATION.md`, `CLI.md`, `UI_DESIGN.md` - пользовательские форматы и интерфейсы.
...
Localization foundation описан в `LOCALIZATION.md`: complete/enabled locales `ru`, `en`, `zh-Hans`, `de`, `es`, `fr`, `ja`, `pt-BR`, `ko`, `uk`, `pl`, `tr`, `ro`, `cs`, `hu` и `bg`; смена языка не переводит сохранённые пользовательские menu data.
