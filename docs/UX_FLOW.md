...
Этот документ фиксирует целевой пользовательский flow для WPF MVP. WPF editor реализует staged editing, add/remove entries, staged выбор `.ico`, drag/drop icon replacement на preview, drag-handle reorder внутри текущей группы, прямой preview из `.ico`, явные Explorer integration controls и user-facing card/list layout.
...
- drag handle для изменения порядка пункта внутри текущей группы;
- поле `DisplayName`;
- поле `DefaultFolderName`;
- поле `GroupName`/`Группа` для optional одноуровневой группы;
- кнопку `Выбрать .ico`;
- drag/drop одного поддерживаемого icon/image file на preview иконки;
...
Если поле пустое, entry остаётся прямо в root menu. `/` и `\` не принимаются, чтобы пользователь не думал, что nested groups уже поддерживаются. Full tree, cross-group drag/drop moves, group/block ordering и group icons остаются future work.
...
IC5a реализует drag-and-drop ordering для entries внутри текущей группы: пользователь тянет entry за маленький handle, drop в верхнюю половину target card вставляет moved entry перед target, drop в нижнюю половину вставляет после target. Изменение применяется только к staged draft, preview/list order обновляется сразу, `Save` сохраняет новый `SortOrder`, а `Discard`/reload возвращает последний saved order.

Cross-group moves, group/block ordering and empty-group drop targets are not implemented in IC5a. They remain planned for a future stage because current groups are derived from flat `GroupName`, not persisted as first-class group entities. Future group polish can add collapse/expand, optional first-class empty groups, duplicate entry/group and group context menus, but current rename/delete group support should not be described as missing.
...
## Icon Picker And Preview Drop Image Conversion
...
Current picker/drop supported files:

```text
Icon/image files (*.ico;*.png;*.jpg;*.jpeg;*.bmp)
```

User can also drop one supported icon/image file onto an entry icon preview:

- `.ico` uses the same staged import-on-save behavior as picker selection.
- `.png`, `.jpg`, `.jpeg` and `.bmp` reuse the existing App icon preparation/conversion service.
- preview updates after successful preparation.
- Save persists a normal `.ico` `IconPath`.

Only one dropped file is accepted. Dropping multiple files, a directory, an unsupported extension or a corrupt/failed image keeps the previous icon unchanged and reports a localized error/status message.
