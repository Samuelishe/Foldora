...
- компактный статус Explorer menu и переход в Settings для управления системными действиями.
...
Главный экран больше не показывает большой блок Explorer integration или danger zone. Он остаётся focused menu editor: title, groups, entries, icons, save/discard, status/errors. Допустимый главный экран status - компактная строка `Explorer menu: Enabled/Disabled` и действие `Manage in Settings`.
