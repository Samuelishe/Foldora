...
Этот документ фиксирует целевой пользовательский flow для WPF MVP. WPF editor реализует staged editing, add/remove entries, staged выбор `.ico`, drag/drop icon replacement на preview, прямой preview из `.ico`, явные Explorer integration controls и user-facing card/list layout.
