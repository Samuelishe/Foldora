...
Этот документ фиксирует целевой пользовательский flow для WPF MVP. Phase 2 уже реализует staged editing, add/remove entries и staged выбор `.ico`; preview и registry controls остаются следующими этапами.
...
User clicks "Выбрать .ico"
  ...
  -> UI shows selected filename/status
