...
`Language` сохраняется в `%AppData%\Foldora\settings.json`. Complete/enabled values: `ru`, `en`; default/fallback: `ru`. Labels/status/defaults WPF идут через App localization catalogs. Смена языка не переводит уже сохранённые menu title, entries, folder names или group names. Новые entries после смены языка используют текущий UI language: `Вид N`/`Новая папка` для `ru`, `View N`/`New folder` для `en`. UI честно предупреждает, что некоторые изменения языка могут применяться после перезапуска.
