...
- поле названия top-level menu, по умолчанию локализованное: `Создать папку` для `ru`, `Create folder` для `en`;
...
`Сбросить меню` требует явного подтверждения в UI. Reset очищает saved entries, возвращает title в localized default-title mode (`Создать папку`/`Create folder`), удаляет только Foldora-owned registry roots, сохраняет `settings.json`, не удаляет AppData root, packs и импортированные `.ico`.
...
`Language` сохраняется в `%AppData%\Foldora\settings.json`. Complete/enabled values: `ru`, `en`. При первом WPF запуске язык выбирается из system UI culture: `ru-*` -> `ru`, `en-*` -> `en`, любой unsupported language -> `en`; затем выбор сохраняется и не определяется повторно на каждом старте. Labels/status/defaults WPF идут через App localization catalogs. Смена языка обновляет untouched/default menu title, но не переводит custom menu title, entries, folder names или group names. Новые entries после смены языка используют текущий UI language: `Вид N`/`Новая папка` для `ru`, `View N`/`New folder` для `en`. UI честно предупреждает, что некоторые изменения языка могут применяться после перезапуска.
