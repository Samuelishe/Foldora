...
- Language setting выбирается в settings UI. First-run language выбирается из system UI culture только для complete/enabled values `ru`, `en`; unsupported system languages получают `en`, а сохранённый ручной выбор не переопределяется.
